...
class AMSlotWindow extends SlotWindow config(AMLadder);
...
function KillGame(int i)
{
	...
	{
		// Are you sure?
		KGQWindow = KillGameQueryWindow(Root.CreateWindow(class'AMKillGameQueryWindow', 100, 100, 100, 100));
		AMKillGameQueryClient(KGQWindow.ClientArea).SlotWindow = Self;
		AMKillGameQueryClient(KGQWindow.ClientArea).SlotIndex = i;
		ShowModal(KGQWindow);
	}
}
...
function RestoreGame(int i)
{
	...
	LadderObj.SkillText = class'AMNewCharacterWindow'.Default.SkillText[LadderObj.TournamentDifficulty];
	
	// Team
	Temp = Right(Saves[i], Len(Saves[i]) - 2);
	Pos = InStr(Temp, "\\");
	Team = int(Left(Temp, Pos));
	LadderObj.Team = class'AMLadderLadder'.Default.LadderTeams[Team];
